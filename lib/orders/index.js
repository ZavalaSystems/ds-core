module.exports = (function (lodash, bilby, R, q, hypermedia, common, lens, monad, db) {
    ...
    var ordersBaseTemplate = lodash.template("/distributor/${d.data.id}/order/${o.data.id}"),
        ...
        createSingleLineItemPrecondition = R.allPredicates(R.flatten([
            lineItemPredicates,
            lookupOrderListPredicates,
            lookupOrderPredicates
        ])),
        reassignOrderPredicates = R.concat(lookupOrderPredicates, [
            R.compose(common.isDefined, R.prop("target")),
            R.compose(common.isNumberLike, R.prop("target"))
        ]),
        reassignOrderPrecondition = R.allPredicates(reassignOrderPredicates),
        ...
        transformOrderID = lens.transformToInt(["orderID"]),
        ...
        transformCreateLineItemInput = R.compose(
            transformLineItemInput,
            transformDistributorID,
            transformOrderID // This captures the distributorID and the orderID from path components
        ),
        transformReassignOrderInput = R.compose(
            transformLookupOrderInput,
            lens.transformToInt(["target"])
        ),
    ...
    return bilby.environment()
        ...
        .property("createSingleLineItemPrecondition", createSingleLineItemPrecondition)
        .property("reassignOrderPrecondition", reassignOrderPrecondition)
        ...
        .property("transformCreateLineItemInput", transformCreateLineItemInput)
        .property("transformReassignOrderInput", transformReassignOrderInput)
