(function (_, req, cypher) {
    ...
    function stripTree(node) {
        ...
        return _.merge({}, _.pick(node, fields), {
            downline: _.map(node.downline, stripTree),
            consultant: node.consultant.data
        });
    }
    ...
    cypher.cypher(query, {})
        .then(function (body) {
            var createObj = _.partial(_.zipObject, body.columns),
                ...
                root = hashSet.get({consultant: {data: {rep: process.argv[2] || "1"}}}),
            ...
            console.log(JSON.stringify(stripTree(withCommissions)));
