/*jslint maxlen: 160*/
module.exports = (function (bilby, R, fortuna) {
    "use strict";
    ...
    var requirements = {
        ...
        mkCurrencyPayload = R.mapObj(mkCurrency),
        ...
        getProgress = function (params) {
            var results = fortuna.run(params.bp, params.distributorID),
                current = results.then(currentVolumes);
            // For each rank, create a goal and a progress
            return current.then(function (c) {
                return {
                    current: mkCurrencyPayload(c),
                    ranks: ranks,
                    goals: R.zipObj(ranks, R.map(mkCurrencyPayload, rankGoalVolumes)),
