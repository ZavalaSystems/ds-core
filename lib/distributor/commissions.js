/*jslint maxlen: 160*/
module.exports = (function (moment, bilby, R, q, rankList, fortuna, db) {
    "use strict";
    ...
    var requirements = {
        ...
        getProgress = function (params, useService) {
            var results = useService ?
                        fortuna.service(params.bp, params.distributorID) :
                        fortuna.batch(params.bp, params.distributorID),
                current = results.then(currentVolumes),
                distributor = db.matchByID(params);
            // For each rank, create a goal and a progress
            return q.spread([current, distributor], function (c, d) {
