...
module.exports = (function (bilby, R, commissions) {
    ...
    var requirements = {
        ...
        convert = R.divide(100), // This does actually produce x/100
    ...
    function getProgress(params) {
        ...
        // TASK pull parts of this out for better testing
        return commissions.run(params.bp, params.distributorID)
            .then(function (results) {
                var pVol = personalVolume(results),
                ...
                return {
                    ...
                    progress: R.map(function (pair) {
                        var targets = pair[1],
                            pvGoal = personalVolumeGoal(targets),
                            gvGoal = groupVolumeGoal(targets),
                            ovGoal = orgVolumeGoal(targets),
                            dGoal = directorsGoal(targets),
                            aGoal = ambassadorsGoal(targets);
                        return [
                            pair[0], {
                                goal: {
                                    // rank: params.goal,
